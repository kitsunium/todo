...
const COMMON = /(password|passwd|qwerty|azerty|123456|abcdef|letmein|welcome|iloveyou|admin|todo)/i;

export function passwordStrength(pw: string): Strength {
  const n = Array.from(pw).length;
  if (n === 0) return { score: 0, label: "", ok: false };
  if (n < PASSWORD_MIN) return { score: 1, label: `${PASSWORD_MIN - n} more character${PASSWORD_MIN - n === 1 ? "" : "s"}`, ok: false };
  if (n > PASSWORD_MAX) return { score: 1, label: "Too long", ok: false };
