  ...
  async function answer(accept: boolean) {
    ...
    try {
      if (accept) {
        ...
        if (onAccepted) {
          toast.success(say("invitation.welcome", { name: g.name }));
          onAccepted();
        } else {
          toast.success(say("invitation.joined", { name: g.name }), {
            action: { label: say("common.open"), onClick: () => navigate(`/app/groups/${g.id}`) },
          });
        }
      } else {
        await ep.invitations.decline(inv.id);
        toast(say("invitation.declined", { name: inv.group.name }));
      }
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl bg-surface p-3 pl-3.5 shadow-card animate-rise max-sm:flex-wrap">
      ...
      <p className="min-w-0 flex-1 text-sm text-fg-2">
        {t.rich("invitation.text", {
          inviter: <span className="font-medium text-fg">{inv.inviter.name}</span>,
          group: (
