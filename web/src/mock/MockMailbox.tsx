...
export default function MockMailbox({ email }: { email?: string }) {
  ...
  return (
    <section className="mt-6 overflow-hidden rounded-xl border border-dashed border-line-strong" aria-label="Mock mailbox">
      ...
      {mails.length === 0 ? (
        <p className="px-3.5 py-3 text-sm text-fg-3">No mail yet.</p>
      ) : (
            ...
            <li key={m.id} className="flex items-center gap-3 px-3.5 py-2.5">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="truncate text-xs text-fg-3">
                  to {m.to} · {formatDistanceToNowStrict(new Date(m.at), { addSuffix: true })}
                </p>
              </div>
              {m.link ? (
                <Link to={m.link} className="shrink-0 rounded-md bg-surface px-2.5 py-1 text-xs font-medium text-fg shadow-card hover:bg-inset">
                  {m.kind === "verify" ? "Verify" : m.kind === "reset" ? "Reset" : "Open"}
                </Link>
              ) : null}
            </li>
          ))}
