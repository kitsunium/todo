...
| **Infrastructure** — its containers, each leading to its view (C4 level 2) | **Domain** — the bounded contexts and how they talk (C4 level 3) |
| ![Infrastructure](docs/studio-infrastructure.png) | ![Domain](docs/studio-domain.png) |
| **Code** — what a node runs, what it touches (C4 level 4) | **Sequence** — what an entry point does, step by step, from its code |
| ![Code](docs/studio-code.png) | ![Sequence](docs/studio-sequence.png) |
| **Requests** — every request as a waterfall, a sequence, its payloads | **States** — a workflow, its population and its instances |
| ![Requests](docs/studio-requests.png) | ![States](docs/studio-states.png) |
| **The daemon** — its phases and its boot sequence | **Runtime** — every loop, by who wrote it: a library, kit, or the product |
| ![Daemon](docs/studio-daemon.png) | ![Runtime](docs/studio-runtime.png) |
| **The HTTP server** — net/http's accept loop and the middleware | **Mail** — every mail the product sent, in dev |
| ![HTTP server](docs/studio-http.png) | ![Mail](docs/studio-mail.png) |
